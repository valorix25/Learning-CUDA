...
__global__ void nf4_dequant_vectorized_kernel(
    ...
    // 将 LUT 加载到 Shared Memory 避免 Constant Memory Bank 冲突
    ...
    if (vec_global_idx + 7 < total_elements) {
        ...
        const uint32_t packed = packed_weights_u32[vec_global_idx / 8];
...
__global__ void nf4_dequant_optimized_kernel(
    ...
    // 在分支发散之前，利用 __ballot_sync 评估哪些线程会进入 fast path
    // 获取一个动态活跃掩码（active_mask），避免在后续使用 0xffffffff 导致死锁
    bool is_fast_path = (vec_global_idx + 7 < total_elements);
    uint32_t active_mask = __ballot_sync(0xffffffff, is_fast_path);

    if (is_fast_path) {
        const uint32_t packed = reinterpret_cast<const uint32_t*>(packed_weights)[vec_global_idx / 8];
        int threads_per_block = (1 << log2_blocksize) >> 3; 
        // 确定当前线程在 Warp 内所属的“领头线程” (Leader Lane)
        // 如果 threads_per_block >= 32, leader_lane 恒为 0
        int leader_lane = (lane_id / threads_per_block) * threads_per_block;
        bool is_leader = (lane_id == leader_lane);

        int64_t block_idx = vec_global_idx >> log2_blocksize;
        float absmax_val_broadcast;
        // 只有领头线程去捞 Metadata，减少对内存控制器的压力
        if (is_leader) {
            int64_t group_idx = block_idx >> 8;
            uint8_t a_idx = absmax_q[block_idx];
            
            float code_v = TypeTraits<T>::cast_to_float(code2[a_idx]);
            float scale_v = TypeTraits<T>::cast_to_float(absmax2[group_idx]);
            absmax_val_broadcast = code_v * scale_v + offset;
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            float val_even = shared_lut[byte_val >> 4] * absmax_val_broadcast;
            float val_odd  = shared_lut[byte_val & 0x0F] * absmax_val_broadcast;

            typename TypeTraits<T>::type2 h2 = TypeTraits<T>::pack(val_even, val_odd);
            out_packed[i] = *reinterpret_cast<uint32_t*>(&h2);
        }

        reinterpret_cast<uint4*>(output)[vec_global_idx / 8] = 
            make_uint4(out_packed[0], out_packed[1], out_packed[2], out_packed[3]);

    } else if (vec_global_idx < total_elements) {
        for (int i = 0; i < 8 && (vec_global_idx + i) < total_elements; i++) {
            int64_t curr = vec_global_idx + i;
            uint8_t p = packed_weights[curr / 2];
            uint8_t n_idx = (curr % 2 == 0) ? (p >> 4) : (p & 0x0F);
            
            int64_t b_idx = curr >> log2_blocksize;
            float cv = TypeTraits<T>::cast_to_float(code2[absmax_q[b_idx]]);
            float sv = TypeTraits<T>::cast_to_float(absmax2[b_idx >> 8]);
            
            output[curr] = TypeTraits<T>::cast_from_float(shared_lut[n_idx] * (cv * sv + offset));
        }
    }
}
...
#define DEFINE_LAUNCHER_TEMPLATE(launcher_name, kernel_name, n_elements)       \
  template <typename T>                                                        \
  torch::Tensor launcher_name(                                                 \
      torch::Tensor packed_weights, torch::Tensor absmax_q,                    \
      torch::Tensor absmax2, torch::Tensor code2,                              \
      int64_t total_elements, int32_t log2_blocksize, float offset) {          \
                                                                               \
    auto th_dtype = std::is_same<T, __half>::value ? torch::kFloat16 : torch::kBFloat16; \
    CHECK_TORCH_TENSOR_DTYPE(absmax2, th_dtype)                                \
    CHECK_TORCH_TENSOR_DTYPE(code2, th_dtype)                                  \
                                                                               \
    auto output = torch::empty({total_elements},                               \
                               torch::dtype(th_dtype).device(packed_weights.device())); \
    int threads = (total_elements < 10240) ? 64 : 256;                         \
    const int blocks = (total_elements + threads * (n_elements) - 1) / (threads * (n_elements)); \
                                                                               \
    kernel_name<T><<<blocks, threads>>>(                                       \
...
DEFINE_LAUNCHER_TEMPLATE(launcher_naive,      nf4_dequant_naive_kernel,      2)
DEFINE_LAUNCHER_TEMPLATE(launcher_vectorized, nf4_dequant_vectorized_kernel, 8)
DEFINE_LAUNCHER_TEMPLATE(launcher_optimized,  nf4_dequant_optimized_kernel, 8)

#define BIND_TEMPLATE_KERNEL(m, launcher_name, T, suffix)                      \
  m.def(TOSTRING(launcher_name##_##suffix), &launcher_name<T>,                 \
        "NF4 Dequantization Kernel (" TOSTRING(suffix) ")");

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    BIND_TEMPLATE_KERNEL(m, launcher_naive,      __half, fp16)
    BIND_TEMPLATE_KERNEL(m, launcher_vectorized, __half, fp16)
    BIND_TEMPLATE_KERNEL(m, launcher_optimized,  __half, fp16)
    
    BIND_TEMPLATE_KERNEL(m, launcher_naive,      __nv_bfloat16, bf16)
    BIND_TEMPLATE_KERNEL(m, launcher_vectorized, __nv_bfloat16, bf16)
    BIND_TEMPLATE_KERNEL(m, launcher_optimized,  __nv_bfloat16, bf16)
}